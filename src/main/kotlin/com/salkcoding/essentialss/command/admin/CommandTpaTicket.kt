...
const val tpaPermissionKey = "com.salk.coding.essentialss.timed_tpa"
            ...
            0 -> {
                ...
                try {
                    ...
                    val user = lp.userManager.getUser(player.uniqueId)
                        ?: throw IllegalStateException("User not found in luckperms")
                    val existingNode =
                        user.nodes.firstOrNull { it.key == tpaPermissionKey && it.hasExpiry() && !it.hasExpired() }
                    ...
                    essentials.server.dispatchCommand(
                        essentials.server.consoleSender,
                        "giveitemone ${player.name} misc MILEAGE"
                    )
                }
            }
        }
